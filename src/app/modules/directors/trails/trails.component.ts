import {
    ChangeDetectionStrategy,
    ChangeDetectorRef,
    Component,
    OnDestroy,
    OnInit,
    ViewEncapsulation,
} from '@angular/core';

import { CommonModule } from '@angular/common';
import { MatDividerModule } from '@angular/material/divider';
import { DirectorsListComponent } from '../list/list.component';
import { MatIconModule } from '@angular/material/icon';
import { ActivatedRoute, Router, RouterLink } from '@angular/router';
import { MatTooltipModule } from '@angular/material/tooltip';
import { MatDrawerToggleResult } from '@angular/material/sidenav';
import { DirectorsService } from '../directors.service';
import { Observable } from 'rxjs';
import { Trail } from 'app/shared/types/trail.types';
import { TrailsOverViewComponent } from 'app/shared/components/trails-overview/trails-overview.component';

@Component({
    selector: 'app-trails',
    templateUrl: './trails.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [CommonModule, MatDividerModule, MatIconModule, RouterLink, MatTooltipModule, TrailsOverViewComponent],
    standalone: true,
})
export class DirectorsTrailsComponent implements OnInit, OnDestroy {
    trails$: Observable<Trail[]>;
    constructor(private _directorsListComponent: DirectorsListComponent, private _router: Router
        , private _route: ActivatedRoute, private _directorsService: DirectorsService,
        private _cdr: ChangeDetectorRef
    ) {}

    ngOnInit(): void {
        this.openDrawer();
        this._route.params.subscribe((params) => {
            this.trails$ = this._directorsService.getDirectorTrails(params?.code);
            this._cdr.detectChanges();
        })
    }

    private openDrawer(): void {
        this._directorsListComponent.matDrawer.open();
    }

    closeDrawer(): Promise<MatDrawerToggleResult> {
        this._router.navigate(['bosedr/directors'])
        return this._directorsListComponent.matDrawer.close();
    }


    ngOnDestroy(): void {}
}
